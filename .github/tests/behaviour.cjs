...
(async()=>{
 ...
 let a=analytics();assert.equal(a.scripts().length,1);assert.equal(a.ctx['ga-disable-G-HEXFX95CM1'],false);assert(a.banner());assert.match(a.banner().innerHTML,/Analytics is on/);a.choice('declined');assert.equal(a.scripts().length,0);assert.equal(a.ctx['ga-disable-G-HEXFX95CM1'],true);assert.equal(a.saved(),'declined');pass('Default loads limited analytics with a notice; turning it off removes the script');
 a=analytics('declined');assert.equal(a.scripts().length,0);assert.equal(a.ctx['ga-disable-G-HEXFX95CM1'],true);assert(!a.banner());pass('A saved objection loads no analytics script and shows no notice');
